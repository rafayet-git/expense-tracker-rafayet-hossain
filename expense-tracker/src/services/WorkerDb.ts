...
function isDbInit(): boolean {
	try {
		return localStorage.getItem('db_init') === DB_VERSION;
	} catch (error) {
...
function setDbInit(): void {
	try {
		localStorage.setItem('db_init', DB_VERSION);
	} catch (error) {
		console.warn('LocalStorage Failed:', error);
	}
}
...
async function openInit(promiser: PromiserFn) {
	...
	try {
		...
		console.warn('Unable to init OPFS:', error);
